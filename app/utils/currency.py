...
async def currency_convert(p_message_list, channel, username, is_private):
    ...
    if isinstance(channel, discord.DMChannel) or channel.name.lower() == "currencies" or channel.permissions_for(username).administrator:
        ...
        if is_private:
            ...
            await currencies_channel.send(embed=embed)
    else:
        await channel.send("If you are not an admin, this function can be used only in currencies channel or in a private chat")